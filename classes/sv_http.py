...
class svHttpCom():
    ...
    def __init__(self, sFullUrl):
        ...
        # self.__g_oCipher = sv_cipher.svCipherMcrypt()
        self.__g_oCipher = sv_cipher.svCipherOpenSsl()
    ...
    def getSecuredUrl( self, dictParams ):
        ...
        try:
            ...
            nIdx = 0
            for e in err.args:
                self.__printDebug( 'http generic error raised arg' + str(nIdx) + ': ' + str( e ))
                nIdx += 1
    ...
    def postUrl( self, dictParams ):
        try:
            if isinstance(dictParams, dict):
                ...
                # self.__printDebug( self.__g_sSubUrl )
                ...
                if oHttpResp.status == 200 and oHttpResp.reason == 'OK':
                    ...
                    self.__printDebug('invalid URL -> status:' + str(oHttpResp.status) + ' reason:' + oHttpResp.reason )
            else:
                self.__printDebug('not a dict type params' )
    ...
    def __printDebug( self, sMsg ):
        if __name__ == '__main__' or __name__ == 'sv_http':
            print( sMsg )
        else:
