...
if __name__ == '__main__': # for class console debugging
	...
	sys.path.append('../../conf')
	import basic_config
elif __name__ == 'classes.sv_mysql' : # for platform running
...
class SvMySql:
    ...
    def __init__(self, sCallingFrom=None):
        ...
        # sMysqlConfigFile = basic_config.ABSOLUTE_PATH_BOT +'/conf/mysql_config.ini'
        try:
            with open(sMysqlConfigFile) as f:
                self.__g_oConfig.read_file(f)
        except IOError:
            self.__printDebug( 'config.ini not exist')
    ...
    def __printDebug( self, sMsg ):
        if __name__ == '__main__' or __name__ == 'sv_mysql' or __name__ == 'classes.sv_mysql':
            print( sMsg )
        else:
    ...
    def __import_pysql(self, s_module_name):
        ...
        try:
            ...
            raise e
    #def __import_pysql(self, module, name):
    #    try:
    #        module = __import__(module, fromlist=[name])
    #        return getattr(module, name)()  # attention to () postfix
    #    except ModuleNotFoundError as e:
    #        raise e
        ...
        if( m ): # if table name is existing in the sql statement
            ...
            if not self.__g_oCursor.rowcount: # table creation
                if __name__ == '__main__':
                    self.__printDebug( "table creation")
                ...
                if __name__ == '__main__':
                    self.__printDebug( "already existing table")
