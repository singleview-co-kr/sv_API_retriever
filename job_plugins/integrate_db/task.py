...
class svJobPlugin():
    ...
    def procTask(self):
        ...
        self.__g_sDataPath = basic_config.ABSOLUTE_PATH_BOT + '/files/' + s_sv_acct_id +'/' + s_acct_title
        self.__g_sFbPnsInfoFilePath = self.__g_sDataPath + '/fb_biz/' + dict_acct_info[s_sv_acct_id]['fb_biz_aid'] + '/contract_pns_info.tsv'

        with sv_mysql.SvMySql('job_plugins.integrate_db') as oSvMysql:
            oSvMysql.setTablePrefix(s_acct_title)
            oSvMysql.initialize()
        
        aNvrAdAcct = dict_acct_info[s_sv_acct_id]['nvr_ad_acct']

        self.__g_sNvrPnsInfoFilePath = self.__g_sDataPath + '/naver_ad/' + s_cid + '/contract_pns_info.tsv'
                    
        if( self.__g_sRetrieveMonth != None ):
            dictDateRange = self.__deleteCertainMonth(s_acct_title)
        else:
            dictDateRange = self.__getTouchDateRange(s_acct_title)
        
        start = dictDateRange['start_date']
        end = dictDateRange['end_date']

        if( start > end ):
            self.__printDebug( 'error - weird raw data last date' )
            return
        ...
        for date in date_generated:
            ...
            self.__compileDailyRecord( s_acct_title, sDate )
        ...
        if aAcctInfo is not None:
            for sSvAcctId in aAcctInfo:
                ...
                self.__g_sDataPath = basic_config.ABSOLUTE_PATH_BOT + '/files/' + sSvAcctId +'/' + sAcctTitle
                
                aNvrAdAcct = aAcctInfo[sSvAcctId]['nvr_ad_acct']
                self.__g_sFbPnsInfoFilePath = self.__g_sDataPath + '/fb_biz/' + aAcctInfo[sSvAcctId]['fb_biz_aid'] + '/contract_pns_info.tsv'
                ...
                for cid in aNvrAdAcct:
                    ...
                    self.__g_sNvrPnsInfoFilePath = self.__g_sDataPath + '/naver_ad/' + cid + '/contract_pns_info.tsv'
                    
                    if( self.__g_sRetrieveMonth != None ):
                        dictDateRange = self.__deleteCertainMonth(sAcctTitle)
                    else:
                        dictDateRange = self.__getTouchDateRange( sAcctTitle )
                    ...
                    for date in date_generated:
                        sDate = date.strftime('%Y-%m-%d')
                        self.__compileDailyRecord( sAcctTitle, sDate )
                        self.__printProgressBar(nIdx + 1, nSentinel, prefix = 'Arrange data:', suffix = 'Complete', length = 50)
                        nIdx += 1
        """
    ...
    def __deleteCertainMonth(self, sAcctTitle):
        ...
        try:
            lstDirectory = os.listdir(self.__g_sDataPath + '/fb_biz' )
            lstFbbizLastDate = []
            for sCid in lstDirectory:
                if sCid == 'alias_info_campaign.tsv':
                    continue
                try:
                    file = open(self.__g_sDataPath + '/fb_biz/' + sCid + '/general.latest', 'r') 
        ...
        dtStartDateRetrieval = datetime.datetime.strptime(sStartDateRetrieval, '%Y-%m-%d')
        dtEndDateRetrieval = datetime.datetime.strptime(sEndDateRetrieval, '%Y-%m-%d')
        return {'start_date': dtStartDateRetrieval,'end_date':dtEndDateRetrieval}

    def __getTouchDateRange(self, sAcctTitle ):
        # get latest retrieval info of naver_ad
        lstDirectory = os.listdir(self.__g_sDataPath + '/naver_ad' )
        lstNaveradLastDate = []
        try:
            ...
            pass
        for sCid in lstDirectory:
            file = open(self.__g_sDataPath + '/naver_ad/' + sCid + '/AD.latest', 'r') 
        ...
        lstDirectory = os.listdir(self.__g_sDataPath + '/adwords' )
        lstAdwordsLastDate = []
        try:
            lstDirectory.remove( 'alias_info_campaign.tsv' )
        except ValueError:
        ...
        for sCid in lstDirectory:
            file = open(self.__g_sDataPath + '/adwords/' + sCid + '/general.latest', 'r') 
            sLatestDate = file.read()
            dtLatest = datetime.date(int(sLatestDate[:4]), int(sLatestDate[4:6]), int(sLatestDate[6:8]))
        ...
        with sv_mysql.SvMySql('job_plugins.integrate_db') as oSvMysql: # to enforce follow strict mysql connection mgmt
            oSvMysql.setTablePrefix(sAcctTitle)
            # init rst dictionary
            dictRst = {}
            ...
            #if( self.__g_bFbProcess ):
            #	lstLastDate.append(max(lstFbbizLastDate))
    ...
    def __compileDailyRecord(self, sAcctTitle, sTouchingDate ):
        try: # validate requsted date
            datetime.datetime.strptime(sTouchingDate, '%Y-%m-%d').strftime('%Y-%m-%d')
        except ValueError:
            self.__printDebug( sTouchingDate + ' is invalid date string' )
            return
    ...
    def __mergeFbGaRaw(self, oSvMysql, sTouchingDate ):
        ...
        for lstMergedLog in self.__g_dictFbMergedDailyLog:
            ...
            if( nRecCnt == 0 ):
                ...
                self.__printDebug( 'fb record with multiple media data on ' + sTouchingDate )
    ...
    def __mergeAdwGaRaw(self, oSvMysql, sTouchingDate ):
        ...
        for lstMergedLog in self.__g_dictAdwMergedDailyLog:
            ...
            if( nRecCnt == 0 ): # add new non-media bounded log, if unable to find related ADW raw log
                ...
                self.__printDebug( 'adw record with multiple media data on ' + sTouchingDate )
                self.__printDebug( lstMergedLog )
                self.__printDebug( self.__g_dictAdwMergedDailyLog[lstMergedLog] )
                self.__printDebug( lstAdwLogDaily )
    ...
    def __mergeYtGaRaw(self, oSvMysql, sTouchingDate ):
        ...
        for lstMergedLog in self.__g_dictYtMergedDailyLog:
            ...
            if( nRecCnt == 0 ):
                ...
                self.__printDebug( 'youtube record with multiple media data on ' + sTouchingDate )
                self.__printDebug( lstMergedLog )
                self.__printDebug( self.__g_dictYtMergedDailyLog[lstMergedLog] )
                self.__printDebug( lstYtLogDaily )
        ...
        for lstMergedLog in self.__g_dictNvadMergedDailyLog:
            ...
            if( sMedia == 'display' and sCamp1st == 'BRS' ):
                ...
                if( len( lstNvadLogDaily ) > 1 ):
                    pass #self.__printDebug( 'NVAD separates NVSHOPPING item so allocate appropriately' )
            else:
            ...
            if( nRecCnt == 0 ):
                ...
                if sMedia == 'display' and sCamp1st == 'BRS':
                    ...
                    self.__printDebug( 'aggregate duplicated nvad log' )
                    self.__printDebug( sTouchingDate )
                    self.__printDebug( lstMergedLog )
                    self.__printDebug( lstNvadLogDaily )
    ...
    def __mergeOtherGaRaw(self, oSvMysql, sTouchingDate ):
        ...
        for lstMergedLog in self.__g_dictOtherMergedDailyLog:
            ...
            if sSource == 'naver' and sRstType == 'PNS':
                #self.__printDebug( '' )
                #self.__printDebug( sTerm + ' on ' + sTouchingDate )
                if( len( dictNvPnsInfo ) > 0 ):
                    ...
                    if( nTouchingDate <= self.__g_nPnsTouchingDate ): # for the old & non-systematic & complicated situation
                        for nIdx in dictNvPnsInfo:
                            #self.__printDebug( 'try to find nick: ' + dictNvPnsInfo[nIdx]['nick'] )
                            nNickIdx = sTerm.find(dictNvPnsInfo[nIdx]['nick'])
                            if nNickIdx > -1 and dictNvPnsInfo[nIdx]['ua'] == sUa:
                                bPnsDetected = True
                                break;
                            #self.__printDebug( nIdx )
                        if( bPnsDetected is False):
                            for nIdx in dictNvPnsInfo:
                                #self.__printDebug( 'try to find term: ' + dictNvPnsInfo[nIdx]['term'] )
                        ...
                        try:
                            ...
                            pass
                            #self.__printDebug( 'no PNS info: ' + sTermForPns )
                            #for sIdx in dictNvPnsInfo:
                            #	self.__printDebug( sIdx )
                            #	self.__printDebug( dictNvPnsInfo[sIdx] )
                            #self.__printDebug( '\n\n' )
        ...
        if( len(dictNvPnsInfo) > 0 ):	
            # self.__printDebug( 'proc remaining dictNvPnsInfo on:' + str(nTouchingDate))
            for sIdx in dictNvPnsInfo:
                if( nTouchingDate <= self.__g_nPnsTouchingDate ): # for the old & non-systematic & complicated situation
        ...
        if( len(dictFbPnsInfo) > 0 ):	
            #self.__printDebug( 'proc remaining dictFbPnsInfo on:' + str(nTouchingDate))
    ...
    def __definePnsCost(self, sTouchingDate, sSource):
        ...
        if sSource == 'naver':
            ...
            self.__printDebug( 'invalid pns info request :' + sSource )
            raise Exception('stop')
        try:
                        ...
                        if row[5] != '-':
                            ...
                            if( dtBeginDate <= dtTouchingDate and dtEndDate >= dtTouchingDate ):
                                ...
                                if( m ): # if valid percent string
                                    ...
                                    self.__printDebug( 'invalid percent string ' + row[4] )
                                    raise Exception('stop')
            ...
            pass
            #self.__printDebug( 'pass ' + self.__g_sNvrPnsInfoFilePath + ' does not exist')
            #raise Exception('stop')
    ...
    def __redefineCost(self, sMedia, sCustomerId, nCost):
        ...
        if( nCost > 0 ):
            ...
            try:
                ...
                self.__printDebug( 'invalid media classifier' + sMedia )
                raise Exception('stop')
            ...
            try:
                ...
                self.__printDebug( 'pass ' + sAgencyInfoFilePath + ' does not exist')
                raise Exception('stop')
            ...
            if( len(aPeriod[0]) > 0 ):
                try: # validate requsted date
                    ...
                    self.__printDebug( 'start date:' + aPeriod[0] + ' is invalid date string' )

            if( len(aPeriod[1]) > 0 ):
                try: # validate requsted date
                    sEndDate = datetime.datetime.strptime(aPeriod[1], '%Y%m%d').strftime('%Y%m%d')
                except ValueError:
                    self.__printDebug( 'end date:' + aPeriod[0] + ' is invalid date string' )
            ...
            if( dtNow < dtBegin ):
                self.__printDebug( 'invalid ' + sMedia + ' agency begin date' )
                raise Exception('stop')
            ...
            if( m ): # if valid percent string
                ...
                self.__printDebug( 'invalid percent string ' + row[2] )
                raise Exception('stop')
            ...
            if( row[3] == 'back' ):
                ...
                self.__printDebug( 'invalid margin type ' + row[3] )
                raise Exception('stop')

            if( sMedia == 'naver_ad' ):
                if( nCost != nFinalCost + nAgencyCost ):
                    self.__printDebug( nCost )
                    self.__printDebug( nFinalCost )
                    self.__printDebug( nAgencyCost )
